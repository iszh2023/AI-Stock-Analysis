...
class MathQuizApp:
    ...
    def apply_palette_now(self, widgets):
        ...
        try:
            ...
            self.style.configure("TCombobox", fieldbackground="#ffffff" if p["bg"] != "#0a0a0a" else p["card"])
    ...
    def draw_gradient(self, canvas, c1, c2):
        ...
        for i in range(steps):
            ...
            y0 = int(h * t)
            canvas.create_rectangle(0, y0, w, y0 + h / steps + 1, outline="", fill=color)
    ...
    def setup_start_screen(self):
        ...
        self.theme_box = ttk.Combobox(form, textvariable=self.theme_var, values=list(PALETTES.keys()),
                                      state="readonly", width=30)
        self.theme_box.grid(row=0, column=1, padx=10, pady=6)
        ...
        self.grade_box = ttk.Combobox(form, textvariable=self.grade_var, values=GRADES, state="readonly", width=30)
        self.grade_box.grid(row=1, column=1, padx=10, pady=6)
        self.grade_box.bind("<<ComboboxSelected>>", self.on_grade_selected)

        self.focus_label = tk.Label(form, text="Focus Mode")
        self.focus_var = tk.StringVar(value="General")
        self.focus_box = ttk.Combobox(form, textvariable=self.focus_var,
                                      values=["General", "Single Subject", "Multiple Subjects"],
                                      state="readonly", width=30)
        self.focus_box.bind("<<ComboboxSelected>>", self.on_focus_mode)
        self.focus_label.grid_remove(); self.focus_box.grid_remove()
        ...
        self.subj_box = ttk.Combobox(self.slide_container, textvariable=self.subj_var, values=[], state="readonly", width=30)
        self.subj_box.bind("<<ComboboxSelected>>", lambda e: setattr(self, "user_selected_subj", True))
        ...
        self.diff_box = ttk.Combobox(form, textvariable=self.diff_var,
                                     values=["Practice (EZ)", "Easy", "Medium", "Hard", "XXX"],
                                     state="readonly", width=30)
        self.diff_box.grid(row=5, column=1, padx=10, pady=6)
        self.diff_box.bind("<<ComboboxSelected>>", self.on_difficulty_change)
        ...
        self.start_btn = tk.Button(btn_row, text="Start Quiz", command=self.start_quiz, width=16, cursor="hand2")
        self.start_btn.pack()
    ...
    def refresh_subject_sources(self):
        ...
        self.subj_box.configure(values=subs_sorted)
        if not self.user_selected_subj:
            self.subj_var.set(self.choose_subject_weighted(subs_sorted))
    ...
    def on_grade_selected(self, _=None):
        ...
        self.focus_box.grid(row=2, column=1, padx=10, pady=6)
        self.slide_container.grid_remove()
    ...
    def on_focus_mode(self, _=None):
        ...
        if mode == "Single Subject":
            ...
            self.subj_box.grid(row=0, column=1, padx=4, pady=(8, 2))
            target_h = 60
    ...
    def start_quiz(self):
        ...
        self.banner.pack(fill="x"); self.draw_gradient(self.banner, self.palette["accent"], darken(self.palette["accent"], 0.4))
        ...
        sub = tk.Label(self.card, text=f"Focus Mode: {self.focus_var.get()} • 3 tries • −2 per miss • first try = max points",
                       font=("Helvetica", 11)); sub.pack(pady=(0, 12))

        self.question_label = tk.Label(self.card, font=("Helvetica", 20, "bold"))
        ...
        self.check_btn = tk.Button(entry_row, text="Check", width=10, cursor="hand2", command=self.check_answer)
    ...
    def show_question(self):
        try:
            ...
            if self.current_q < len(self.questions):
                ...
                self.question_label.config(text=f"Subject: {subj}  —  Q{self.current_q + 1}:  {text}")
            ...
            self.question_label.config(text="(whoops) loading question; please be patient...")
    ...
    def check_answer(self):
        ...
        if is_correct:
            ...
            # scoring
            points = max(0, 10 - 2 * self.attempts)
            ...
            if PIL_OK:
                ...
                self.set_progress(self.current_q, "green")

            if self.attempts == 0:
                msg = f"Perfect! +{points} points (first try)"
                color = self.palette["accent"]
            elif self.attempts == 1:
                msg = f"Nice! +{points} points"
                color = "#22c55e"
            else:
                msg = f"Got it. +{points} points"
                color = "#f59e0b"
            self._award_and_advance(msg, color)
        else:
            self.attempts += 1
            if self.attempts == 1 and len(hints) >= 1:
                self.feedback_label.config(text=f"Hint 1: {hints[0]}", fg="#f59e0b")
            elif self.attempts == 2 and len(hints) >= 2:
                self.feedback_label.config(text=f"Hint 2: {hints[1]}", fg="#f59e0b")
            else:
                self.give_up()
    ...
    def give_up(self):
        ...
        self._award_and_advance(f"Gave up. 0 points. Correct: {corr}", "#ef4444")
